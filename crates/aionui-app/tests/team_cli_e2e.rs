...
async fn team_capabilities_prints_contract_without_runtime_env() {
    let output = team_command()
        ...
        .env_remove("CARBONFUSION_RUNTIME_TOKEN")
        .output()
        .await
        .unwrap();
...
async fn tool_command_rejects_forged_identity_fields_before_http_call() {
    let mut child = team_command()
        .args(["send-message"])
        .env("CARBONFUSION_BASE_URL", "http://127.0.0.1:9")
        .env("CARBONFUSION_CONVERSATION_ID", "conv-1")
        .env("CARBONFUSION_USER_ID", "user-1")
        .env("CARBONFUSION_RUNTIME_TOKEN", "token-1")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
