...
fn extract_csrf_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find(|s| s.starts_with("carbonfusion-csrf-token="))
        .map(|s| {
            s.strip_prefix("carbonfusion-csrf-token=")
                .unwrap()
                .split(';')
                .next()
                .unwrap()
...
fn extract_session_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .iter()
        .filter_map(|v| v.to_str().ok())
        .find(|s| s.starts_with("carbonfusion-session="))
        .and_then(|s| {
            let value = s.strip_prefix("carbonfusion-session=")?.split(';').next()?.to_owned();
...
fn get_with_cookie(uri: &str, token: &str) -> Request<Body> {
    Request::builder()
        .method("GET")
        .uri(uri)
        .header("cookie", format!("carbonfusion-session={token}"))
        .body(Body::empty())
        .unwrap()
}
...
async fn t13_1_authorization_header_takes_priority() {
    ...
    let req = Request::builder()
        ...
        .uri("/api/auth/user")
        .header("authorization", format!("Bearer {token}"))
        .header("cookie", "carbonfusion-session=invalid_token")
        .body(Body::empty())
        .unwrap();
    let resp = app.oneshot(req).await.unwrap();
