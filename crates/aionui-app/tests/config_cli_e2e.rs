...
async fn config_mcp_oauth_logout_reads_status_before_and_after_write() {
    ...
    let mut child = config_command()
        ...
        .env("CARBONFUSION_CONVERSATION_ID", "conv-mcp")
        .env("CARBONFUSION_USER_ID", "user-mcp")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_capabilities_prints_agent_readable_contract_without_runtime_env() {
    ...
    assert_eq!(
        ...
        "resolve via CARBONFUSION_CONVERSATION_ID"
    );
...
async fn config_provider_update_reads_collection_before_and_after_write() {
    ...
    let mut child = config_command()
        ...
        .env("CARBONFUSION_BASE_URL", &base_url)
        .env("CARBONFUSION_CONVERSATION_ID", "conv-provider")
        .env("CARBONFUSION_USER_ID", "user-provider")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_conversation_rename_patches_name_and_reads_resource_before_and_after() {
    ...
    let mut child = config_command()
        ...
        .env("CARBONFUSION_USER_ID", "user-rename")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_external_paths_add_reads_collection_before_and_after_write() {
    ...
    let mut child = config_command()
        ...
        .env("CARBONFUSION_BASE_URL", &base_url)
        .env("CARBONFUSION_CONVERSATION_ID", "conv-skill")
        .env("CARBONFUSION_USER_ID", "user-skill")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_payload_commands_resolve_current_conversation_and_user_selectors() {
    ...
    let mut child = config_command()
        ...
        .env("CARBONFUSION_BASE_URL", &base_url)
        .env("CARBONFUSION_CONVERSATION_ID", "conv-selector")
        .env("CARBONFUSION_USER_ID", "user-selector")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_agent_custom_update_reads_agent_id_from_stdin() {
    ...
    let mut child = config_command()
        ...
        .env("CARBONFUSION_BASE_URL", &base_url)
        .env("CARBONFUSION_CONVERSATION_ID", "conv-agent")
        .env("CARBONFUSION_USER_ID", "user-agent")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn config_cron_job_skill_save_reads_job_id_from_stdin() {
    ...
    let mut child = config_command()
        ...
        .env("CARBONFUSION_BASE_URL", &base_url)
        .env("CARBONFUSION_CONVERSATION_ID", "conv-cron")
        .env("CARBONFUSION_USER_ID", "user-cron")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
fn builtin_config_skills_use_config_cli_not_python_or_cron_helper() {
    ...
    assert!(aionui_config.contains("\"$CARBONFUSION_HELPER_BIN\" config context"));
    assert!(aionui_config.contains("\"$CARBONFUSION_HELPER_BIN\" config capabilities"));
    assert!(aionui_config.contains("assistant_id\": \"current"));
    for command in [
        "\"$CARBONFUSION_HELPER_BIN\" config mcp servers",
        "\"$CARBONFUSION_HELPER_BIN\" config providers",
        "\"$CARBONFUSION_HELPER_BIN\" config settings",
        "\"$CARBONFUSION_HELPER_BIN\" config agents",
        "\"$CARBONFUSION_HELPER_BIN\" config cron jobs",
        "\"$CARBONFUSION_HELPER_BIN\" config skills external-paths",
    ] {
        assert!(
    ...
    assert!(cron.contains("\"$CARBONFUSION_HELPER_BIN\" config cron current list"));
