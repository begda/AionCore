...
use \"$CARBONFUSION_HELPER_BIN\" team capabilities or \"$CARBONFUSION_HELPER_BIN\" team help to inspect the Team contract,\n\
...
use the Team CLI fallback through \"$CARBONFUSION_HELPER_BIN\" team ... commands to continue Team coordination.\n\n\
...
fn render_cli_usage(role: TeamPromptRole, descriptors: &[TeamToolDescriptor]) -> String {
    let mut text = String::from(
        "You MUST use CarbonFusion Team CLI for ALL team coordination:\n\
\"$CARBONFUSION_HELPER_BIN\" team ...\n\n\
Run \"$CARBONFUSION_HELPER_BIN\" team capabilities when you need command names,\n\
stdin JSON schema, required fields, enum values, permissions, examples,\n\
or error meanings.\n\n\
Run \"$CARBONFUSION_HELPER_BIN\" team help when you need a short readable guide.\n\n\
...
mod tests {
    ...
    fn cli_teammate_usage_excludes_lead_only_tools() {
        ...
        assert!(usage.contains("\"$CARBONFUSION_HELPER_BIN\" team"));
    ...
    fn mcp_lead_usage_includes_lead_tools() {
        ...
        assert!(usage.contains("\"$CARBONFUSION_HELPER_BIN\" team capabilities"));
